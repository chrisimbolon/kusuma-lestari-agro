 ...
 * ║                                                               ║
 * ║  Fixes applied:                                               ║
 * ║  1. useAuth().login is async fn — not a mutation object.     ║
 * ║     Replaced login.mutate() → await login()                  ║
 * ║  2. Payload field: username → email (matches Django backend) ║
 * ║  3. Loading + error state managed locally via useState        ║
 * ╚═══════════════════════════════════════════════════════════════╝
 */
  ...
  <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#f0ede6", fontFamily:"'Instrument Sans',sans-serif", padding:24 }}>
    ...
    <div className="auth-card" style={{ background:"#fff", borderRadius:20, padding:"44px 40px", width:"100%", maxWidth:440, boxShadow:"0 4px 6px rgba(0,0,0,0.04),0 20px 60px rgba(0,0,0,0.08)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:36 }}>
        ...
        <span style={{ fontFamily:"'DM Serif Display',serif", fontSize:22, fontWeight:400, color:"#1a2e1a", letterSpacing:"-0.01em" }}>
          Sistem Kusuma Lestari Agro
        </span>
      </div>
      {children}
    </div>
...
function Field({ label, id, type = "text", placeholder, value, onChange, icon, rightIcon, focused, onFocus, onBlur, hasError }: {
  ...
  return (
    <div>
      <label style={{ display:"block", fontSize:13, fontWeight:600, color:"#2d3a2d", marginBottom:7 }}>
        {label}
      </label>
      <div
        className={`kla-field${focused ? " focused" : ""}${hasError ? " has-error" : ""}`}
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // It calls authApi.login(), sets auth state, and navigates to /dashboard.
  ...
  return (
    <AuthShell>
      ...
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily:"'DM Serif Display',serif", fontSize:34, fontWeight:400, color:"#1a1a14", margin:"0 0 6px", letterSpacing:"-0.02em" }}>
          Masuk
        </h1>
        <p style={{ color:"#9a8f7a", fontSize:14, margin:0 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:18 }}>
        ...
        <Field
          label="Email" id="kla-email" type="email" placeholder="indra@kla.com"
        ...
        <Field
          label="Kata Sandi" id="kla-password"
